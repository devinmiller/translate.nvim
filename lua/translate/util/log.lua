...
---@param ... any: the arguments of the formatted string.
---@private
function log.info(scope, str, ...)
  return log.notify(scope, vim.log.levels.INFO, true, str, ...)
end
...
function log.notify(scope, level, verbose, str, ...)
  if not verbose and _G.Translate.config ~= nil and not _G.Translate.config.debug then
    return
  end
  ...
  vim.notify(
    string.format("[translate.nvim@%s] %s", scope, string.format(str, ...)),
    level,
...
function log.warn_deprecation(options)
  ...
  if uses_deprecated_option then
    ...
    log.notify(
      ...
      vim.log.levels.WARN,
      true,
      "use `:h Translate.options` to read more."
    )
  end
end
